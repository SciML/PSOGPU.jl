using ParallelParticleSwarms, StaticArrays, SciMLBase, Test, LinearAlgebra, Random, KernelAbstractions
...
cache = init(prob, ParallelParticleSwarms.HybridPSO(; local_opt = ParallelParticleSwarms.BFGS(), backend = CPU()))

reinit!(cache)

cache = init(prob, ParallelParticleSwarms.HybridPSO(; local_opt = ParallelParticleSwarms.LBFGS(), backend = CPU()))
